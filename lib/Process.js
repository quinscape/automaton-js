...
exports.getGraphQLMethodType = getGraphQLMethodType;
...
function renderCurrentView() {
    ...
    return _react2.default.createElement(
        ...
        _react2.default.createElement(
            _domainqlForm.FormConfigProvider,
            {
...
var Process = exports.Process = function () {
    ...
    _createClass(Process, [{
        ...
        value: function transition(name, context) {
            ...
            var confirmation = transition.confirmation;


            if (confirmation) {
                var message = confirmation(context);
        ...
        key: "getTransition",
        value: function getTransition(name) {
            var access = this[secret];
...
function getGraphQLMethodType(gqlMethod) {
    if (!gqlMethod) {
        throw new Error("Need gqlMethod");
    }

    var queryType = _config2.default.inputSchema.getType("QueryType");

    var queryFieldType = getFieldTypeByName(queryType.fields, gqlMethod);
    if (queryFieldType) {
        return queryFieldType;
    }

    var mutationType = _config2.default.inputSchema.getType("MutationType");

    var mutationFieldType = getFieldTypeByName(mutationType.fields, gqlMethod);
...
function inject(scope, injections) {
    ...
    for (var i = 0; i < scopeKeys.length; i++) {
        ...
        if (prop instanceof _GraphQLQuery2.default) {
            ...
            var _names = _slicedToArray(names, 1),
                gqlMethod = _names[0];

            var type = getGraphQLMethodType(gqlMethod);

            var injectionValue = result[gqlMethod];
            //console.log("inject", name, "with", gqlMethod, injectionValue, "type = ", JSON.stringify(type));

            try {
                var converted = (0, _domain.getWireFormat)().convert(type, injectionValue, true);

                //console.log("SCOPE:" + name , "=", converted);

                (0, _mobx.set)(scope, _name, converted);
            } catch (e) {
                var msg = "Error converting '" + _name + "'";
                console.error(msg, e);
