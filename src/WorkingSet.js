...
function getFieldChangesForObject(status, domainObject, base, mergePlan)
{
...
function hasFieldChanges(status, domainObject, base, mergePlan)
{
...
function getChangesForNewObject(registration)
{
    const { domainObject, typeName } = registration;
    const { mergePlan } = registration.workingSet
...
function getFieldChanges(registrations, mergePlan)
{
    ...
    for (let entry of registrations.values())
    {
        ...
        if (isNew)
        {
            fieldChanges = getChangesForNewObject(entry)
        }
 ...
 * @param {MergePlan} mergePlan                 merge plan instance
 */
function getManyToManyChanges(changesAndDeletions, registrations, mergePlan)
{
    ...
    for (let i = 0; i < changesAndDeletions.length; i++)
    {
        ...
        if (linkTypes)
        {
            ...
            const { domainObject: linkInstance } = registrations.get(changeKey(change.type, change.id.value))
            ...
            for (let j = 0; j < linkTypes.length; j++)
            {
                ...
                for (let [key, { domainObject, status }] of registrations.entries())
                {
    ...
    {
        // get the full object for the change
        const { domainObject } = registrations.get(key)
 ...
 * @param {MergePlan} mergePlan     merge plan instance
 */
function hasManyToManyChanges(entityType, entityId, registrations, mergePlan)
{
    for (let entry of registrations.values())
    {
...
function stringArrayEquals(valueA, valueB)
{
    if (valueA === null)
    {
        return valueB === null
    ...
    if (valueA.length !== valueA.length)
    {
        return false;
    }
...
function checkUpdateEquality(a,b)
{
    if (a.length !== b.length)
    {
        return false;
    ...
    for (let i = 0; i < a.length; i += 3)
    {
        ...
        else if (scalarType === LIST_OF_STRINGS)
        {
            return stringArrayEquals(valueA, valueB)
        }
    ...
    collectManyToManyUpdates(updates)
    {
        ...
        for (let i = 0; i < embedded.length; i++)
        {
            ...
            if (entry.isManyToMany)
            {
                ...
                const linkedBase = base[linkFieldName].map( o => getLinkedObjectId(o, otherRelation.leftSideObjectName) );
                const linked = domainObject[linkFieldName].map( o => getLinkedObjectId(o, otherRelation.leftSideObjectName) );

                linkedBase.sort()
                linked.sort()

                if (!arrayEqual(linkedBase, linked))
                {
                    updates.push(
                        linkFieldName,
                        LIST_OF_STRINGS,
...
function getLinkedObjectId(object, field)
{
...
export default class WorkingSet {
    ...
    apply(resolutions, conflicts)
    {
        ...
        const defaultApply = () => {
            ...
            for (let i = 0; i < resolutions.length; i++)
            {
                ...
                const { domainObject, base } = registrations.get(changeKey(type, id.value));
    ...
    merge(attempt = 1)
    {
        ...
        let changes = getFieldChanges(registrations, mergePlan);
        ...
        changes = changes.concat(getManyToManyChanges(changesAndDeletions, registrations, mergePlan));
        ...
        return this._mergeQuery.execute(vars)
            .then(({ mergeWorkingSet: result }) => {
                ...
                else
                {
                    ...
                    {
                        console.info("STARTING MERGE: scenario = ", toJS(
                            {
